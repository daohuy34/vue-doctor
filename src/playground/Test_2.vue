...
<!-- ============================================================
  COMPONENT CON — minh họa các anti-pattern no-mutate-props
  Trong thực tế đây sẽ là file riêng, để ở đây cho tiện test
  ============================================================ -->
<script>
// ── Options API component — BAD CASES ─────────────────────────────────────
export default {
  name: 'TodoItemBad',

  props: {
    // prop object form với type đầy đủ
  ...
  methods: {
    // BAD: gán thẳng vào prop object
    markDone() {
      this.todo.done = true;           // mutation nested prop
    },

    // BAD: gán đè toàn bộ prop
    resetTodo() {
      this.todo = null;                // mutation direct prop
    },

    // BAD: mutation qua computed index
    replaceFirst() {
      this.items[0] = { id: 99, text: 'replaced' };  // mutation by index
    },

    // BAD: array mutating methods trên prop
    addItem(item) {
      this.items.push(item);           // push
    },

    removeFirst() {
      this.items.shift();              // shift
    },
    ...
    removeAt(index) {
      this.items.splice(index, 1);     // splice
    },

    sortItems() {
      this.items.sort((a, b) => a.id - b.id);   // sort
    },
    ...
    // BAD: mutation bên trong lifecycle hook
    mounted() {
      this.modelValue = 'init';        // mutation trong hook
    },

    // BAD: mutation prop string
    clearValue() {
      this.modelValue = '';            // mutation string prop
    },

    // BAD: increment số
    increment() {
      this.count = this.count + 1;     // mutation number prop
    },

    // GOOD: đọc prop không mutate — không bị flag
    getLabel() {
      return this.todo.text.toUpperCase();
    },

    // GOOD: emit thay vì mutate — không bị flag
    updateText(newText) {
      this.$emit('update:modelValue', newText);
    },

    // GOOD: mutate biến local data, không phải prop — không bị flag
    toggleLocalFlag() {
  ...
  data() {
    return {
      localFlag: false,          // local data — mutation ở đây là OK
      localItems: [],            // local copy — OK khi mutate
    };
  },
};
</script>
...
<script setup>
// ── Composition API (<script setup>) — BAD CASES ──────────────────────────
import { defineProps, defineEmits, withDefaults } from 'vue';

// defineProps array form
const props = defineProps(['modelValue', 'items', 'config']);

// BAD: gán trực tiếp vào prop
props.modelValue = 'mutated';

// BAD: gán nested
props.config.theme = 'dark';

// BAD: array mutation methods
props.items.push({ id: 1 });
...
// GOOD: chỉ đọc — không bị flag
console.log(props.modelValue);
const label = props.config?.title ?? 'default';

// GOOD: emit — không bị flag
const emit = defineEmits(['update:modelValue']);
emit('update:modelValue', 'new value');
</script>

<script setup lang="ts">
// ── TypeScript generic defineProps — BAD CASES ────────────────────────────
interface TodoItem {
  id: number;
  text: string;
  done: boolean;
...
// BAD: mutation qua withDefaults proxy
tsProps.title = 'hacked';
tsProps.todo.done = true;
tsProps.tags.push('new-tag');
</script>